	...
	<cfscript>
		...
		for (i = 1; i lte 99; i = i + 1) {
			try {
				...
				//	cf_log('404', 'A. sizeOfLatencyAR = [#sizeOfLatencyAR#], i = [#i#], latencyAR[#i#].etMs = [#latencyAR[i].etMs#], leastLatency = [#leastLatency#]');
				...
				//	cf_log('404', 'B. i = [#i#], leastLatency = [#leastLatency#], serverNumWithLeastHits = [#serverNumWithLeastHits#]');
				//	writeOutput('Considering Server#i#<br>');
				}
			} catch (Any e) {
			}
		}
	//	cf_log('404', 'C. leastLatency = [#leastLatency#], serverNumWithLeastHits = [#serverNumWithLeastHits#]');
