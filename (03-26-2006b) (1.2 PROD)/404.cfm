...
<cftry>
	...
	<cfscript>
		...
		if ((bool_debugMode) AND (bool_allowDebug)) writeOutput('etMs1 = [#etMs1#]<br>');
	</cfscript>
	...
	<cfscript>
		...
		if ((bool_debugMode) AND (bool_allowDebug)) writeOutput('etMs2 = [#etMs2#]<br>');
	</cfscript>
	...
	<cfscript>
		...
		if (serverNumWithLeastHits eq 1) {
			torf = 'F';
			redirectToURL = 'http://#url1##CGI.SCRIPT_NAME#?#CGI.QUERY_STRING#';
		...
		if (serverNumWithLeastHits eq 2) {
			torf = 'F';
			redirectToURL = 'http://#url2##CGI.SCRIPT_NAME#?#CGI.QUERY_STRING#';
